...
import { getNewAlerts, deleteAllAlerts } from 'redux/slices/alert';
...
//     text: 'partially filled issue happened!',
//     isRead: false,
//   },
// ];
export default function NotificationsPopover() {
  ...
  const totalUnRead = alerts.filter((item) => item.isRead === false).length;
        ...
        <Scrollbar sx={{ height: { xs: 340, sm: 'auto' } }}>
          ...
          >
            {alerts.map((alert, index) => (
              <NotificationItem key={index} alert={alert} />
            ))}
          </List>
...
function NotificationItem({ alert }) {
  const { title, text, flightId, stepId } = alert;
  ...
  return (
    ...
    >
      ...
      <ListItemText
        disableTypography
        primary={`In Step ${stepId}, ${text}`}
        secondary={
          <Stack direction="row" sx={{ mt: 0.5, typography: 'caption', color: 'text.disabled' }}>
            <Iconify icon="eva:clock-fill" width={16} sx={{ mr: 0.5 }} />
            <Typography variant="caption">{fToNow(alert.createdAt)}</Typography>
          </Stack>
        }
      />
    </ListItemButton>
